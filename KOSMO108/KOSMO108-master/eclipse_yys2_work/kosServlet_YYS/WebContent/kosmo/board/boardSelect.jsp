...
<script type="text/javascript">
	$(document).ready(function(){	
		...
		$("#bpwChk").click(function(){
			// <input type="text" style="width:100%; background-color:#e6e6e6;" class="bpw" name="bpw" id="bpw"
			alert("비밀번호를 체크하도록 하겠습니다. >>> : ");
			...
			function whenSuccess(resData){
				...
				if(sVal=='PW_GOOD'){
					...
					$("#bpw").css("background-color", "#e6e6e6");
					$("#bmemo").focus();
					$("#bpw").prop("readonly", true);
					$(".bsubject").prop("readonly", false);
					$(".bsubject").css("background-color", "#e6ffe6");
					$(".bmemo").prop("readonly", false);
					$(".bmemo").css("background-color", "#e6ffe6");
...
<form class="f1" id="f1" name="f1">
		<table>
			...
			<td><input style="width:170%; background-color:#e6e6e6;" type="text" class="bnum" id="bnum" name="bnum" required maxlength="100" placeholder="글 번호" value="<%= _bvo.getBnum() %>" readonly></td>
			</tr>		
			<tr>
			<!--  글제목 -->
			<td style="background-color:#e6ffe6">글제목(수정가능)</td>
			<td><input style="width:170%; background-color:#e6e6e6;" type="text" class="bsubject" id="bsubject" name="bsubject" maxlength="30" placeholder="수정할 글제목" value="<%=_bvo.getBsubject() %>" readonly></td>
			</tr>
			<tr>
			<!--  글쓴이 -->
			<td style="background-color:#e6ffe6">글쓴이(수정불가)</td>
			<td><input style="width:170%; background-color:#e6e6e6;" type="text" class="bwriter" id="bwriter" name="bwriter" maxlength="30" placeholder="글쓴이 수정" value="<%=_bvo.getBwriter() %>" readonly></td>
			</tr>	
			<tr>
			<!--  비밀번호 -->
			<td style="background-color:#e6ffe6">비밀번호(수정불가)</td>
			<td><input type="password" style="width:100%; background-color:#e6ffe6";" class="bpw" name="bpw" id="bpw"
			...
			<td><textarea style="width:170%; height:200%; background-color:#e6e6e6;" class="bmemo" id="bmemo" name="bmemo" maxlength="2000" width="500px" height="500px" value="<%= _bvo.getBmemo() %>" placeholder="수정할 글내용" value="<%= _bvo.getBmemo()  %>" readonly></textarea></td>
			</tr>
			<tr>
			<!--  회원여부 -->
			<td style="background-color:#e6ffe6">회원여부(수정불가)</td>
			<td><input style="width:170%; background-color:#e6e6e6;" type="text" class="deleteyn" id="deleteyn" name="deleteyn" maxlength="30" placeholder="회원여부" value="<%=_bvo.getDeleteyn() %>" readonly></td>
			</tr>
			<tr>
			<!--  등록일 -->
			<td style="background-color:#e6ffe6">등록일(수정불가)</td>
			<td><input style="width:170%; background-color:#e6e6e6;" type="text" style="width:250px;" value="<%= _bvo.getInsertdate() %>"></td>
			</tr>
			<tr>
			<!--  수정일 -->
			<td style="background-color:#e6ffe6">수정일(수정불가)</td>
			<td><input style="width:170%; background-color:#e6e6e6;" type="text" class="updatedate" id="updatedate" name="updatedate" maxlength="30" placeholder="수정일" value="<%=_bvo.getUpdatedate() %>" readonly></td>
			</tr>
			<hr>
			<!-- 버튼 -->
