...
<script type="text/javascript">
	$(document).ready(function(){
		// alert("kosmoRboardForm jsp 페이지 로딩 완료 + jQuery ready~");
		...
		$(document).on("click", "#rinsertBtn", function(){
			alert("등록하기 버튼을 클릭하셨습니다 >>> : ");
			
			let insertURL="kosmoRboardInsert.yys";
			let method="POST";
			let dataParam={
					kbnum: $("#kbnum").val(),
					ksbname:$("#rkbname").val(),
		...
		$(document).on("click", ".deleteBtn", function(){
			...
			let deleteURL="kosmoRboardDelete.yys";
			let method="POST";
			let dataParam={
		...
		function selectAll(){
			...
			let selectAllURL="kosmoRboardSelectAll.yys";
			let method="POST";
			let dataParam={
				kbnum:$("#kbnum").val(),
			};
			...
			function whenSuccess(resData){
				alert("whenSucess resData >>> : "+resData);
				...
				for(let i=0; i<v.length; i++){
					console.log(v[i]);
					let vv=v[i].split(",");
					console.log("vv.length >>> : "+vv.length);
					let j=0;
					for (; j<vv.length-1; j++){
